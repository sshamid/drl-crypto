...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    insample_turbulence = df[(df.datadate<20190101)]
    insample_turbulence = insample_turbulence.drop_duplicates(subset=['datadate'])
    
    insample_turbulence_threshold = np.quantile(insample_turbulence.turbulence.values, .95)
    print("insample turbulence_threshold: ", insample_turbulence_threshold)
    start = time.time()
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        ## training env
        train = data_split(df, start=df.datadate.min(), end=unique_trade_date[i - rebalance_window - validation_window])
        print(f'training shape : {train.shape} and data columns {train.columns.tolist()}')
        ...
        print("======Model training from: ", df.datadate.min(), "to ",
              unique_trade_date[i - rebalance_window - validation_window])
        # print("training: ",len(data_split(df, start=20090000, end=test.datadate.unique()[i-rebalance_window]) ))
        # print("==============Model Training===========")
        print("======A2C Training========")
        model_a2c = train_A2C(env_train, model_name="A2C_30k_dow_{}".format(i), timesteps=150000)
...
#         print("======PPO Training========")
#         model_ppo = train_PPO(env_train, model_name="PPO_100k_dow_{}".format(i), timesteps=150000)
...
#         print("======DDPG Training========")
#         model_ddpg = train_DDPG(env_train, model_name="DDPG_10k_dow_{}".format(i), timesteps=150000)
